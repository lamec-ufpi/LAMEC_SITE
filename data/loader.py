...
def load_proceedings_all() -> dict:
    ...
    if proc_dir.exists():
        for f in proc_dir.glob("*.json"):
            ...
            # Normalização Blindada (Evita duplicar prefixo se recarregar)
            ...
            for cat in raw.get("categories", []):
                for article in cat.get("articles", []):
                    ...
                    if pdf:
                        ...
                        article["pdf_path"] = None # <-- CORREÇÃO AQUI (Antes era "#")
                    
                    # Trata null, chave inexistente ou string vazia, convertendo tudo para None
