document.addEventListener('DOMContentLoaded', () => {
    ...
    initSlider('simcompi-1-slider', [
        "/static/images/simcompi/simcompi 1/slide/boi bumba.jpg",
        "/static/images/simcompi/simcompi 1/slide/janta com profs no i simcompi.jpg",
        "/static/images/simcompi/simcompi 1/slide/mesa de honra do i simcompi.jpg",
        "/static/images/simcompi/simcompi 1/slide/kurka palestra no i simcompi.jpg",
        "/static/images/simcompi/simcompi 1/slide/palestrantes e organizadores do i simcompi no encerramento.jpg",
        "/static/images/simcompi/simcompi 1/slide/poster-presentation no i simcompi.jpg"
    ]);

    initSlider('simcompi-2-slider', [
        "/static/images/simcompi/simcompi 2/slide/pessoas-assistindo-o-ii-simcompi.png",
        "/static/images/simcompi/simcompi 2/slide/feira1.png",
        "/static/images/simcompi/simcompi 2/slide/palestra-do-prof-fabro.png",
        "/static/images/simcompi/simcompi 2/slide/professores-e-organizacao-no-encerramento.png"
    ]);

    // Evita o scroll/pulo da página no slider CSS da Home
    document.querySelectorAll('.slider-nav a').forEach(link => {
        link.addEventListener('click', function(e) {
            ...
            if (targetSlide) {
                const originalId = targetSlide.id;
                targetSlide.id = originalId + '-temp';
                window.location.hash = targetId;
                targetSlide.id = originalId;
            }
        });
    });
...
function getPdfUrl(pdfLink) {
    ...
    // Se for link externo, não mexe
    if (url.startsWith('http://') || url.startsWith('https://')) {
        return escapeHtml(url);
    }
    
    // A MÁGICA AQUI: Remove '../', barras repetidas e a palavra 'static' 
    url = url.replace(/\.\.\//g, '')       // Remove todos os ../
             .replace(/^\/+/, '')          // Remove barras do início
             .replace(/^static\//, '');    // Remove a palavra static/ do início, se tiver

    // Retorna o link limpo com /static/ no começo
